    ...
    public class Register : IEndpoint
    {
        ...
        private static async Task<IResult> Handler(Request request, ApplicationDbContext context, UserManager<Account> userManager, SignInManager<Account> signInManager, ClaimsPrincipal user)
        {
            ...
            if (result.Succeeded)
            {
                ...
                bool emailResponse = await EmailSender.SendConfirmEmail(createduser.Email, "Xác nhận Email tài khoản","Xác nhận tài khoản bạn vừa mới đăng ký!",ConfirmLink,"Xác nhận");
                if (!emailResponse) {
